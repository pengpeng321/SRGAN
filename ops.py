...
# 返回卷积层https://www.cnblogs.com/qggg/p/6832342.html
def down_sample_layer(input_x):
    # input_x 8 256 256 3
    # Tensor("Const_1:0", shape=(4, 4, 3, 3), dtype=float32)
    # 卷积滤波器窗口大小K = 4，
    # 卷积水平滑动和垂直滑动步长值均为K = 4 （由参数strides确定）
    ...
    downscaled = tf.nn.conv2d(input_x, weight, strides=[1, K, K, 1], padding='SAME')
